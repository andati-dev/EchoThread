...
export default function Register({ onRegister }: { onRegister?: (token: string) => void }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addToast({ type: 'error', message: err?.response?.data?.error || 'Registration failed' })
  ...
  return (
    <form className="max-w-md mx-auto p-4 bg-white rounded" onSubmit={submit}>
      ...
      <div className="mb-2">
        <input className="w-full border px-2 py-1" placeholder="Name (optional)" value={name} onChange={(e) => setName(e.target.value)} />
      </div>
      <div className="mb-2">
      ...
      <div>
        <button className="px-3 py-1 bg-green-600 text-white rounded" disabled={loading}>{loading ? 'Creating…' : 'Create account'}</button>
      </div>
    </form>
  )
}
